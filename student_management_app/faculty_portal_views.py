...
from student_management_app.models import Attendance, AttendanceReport, Courses, CustomUser, FeedBackStaffs, LeaveReportStaff, SessionYearModel, Staffs, StudentResult, Students, Subjects


def render_staff_dashboard(request):
...
def render_attendance_capture(request):
    subjects = Subjects.objects.filter(staff_id=request.user.id).order_by('subject_name')
    session_years = SessionYearModel.objects.all().order_by('-session_start_year')
    if not subjects.exists():
        messages.warning(request, "No subjects are assigned to your account yet. Please contact admin.")
    if not session_years.exists():
        messages.warning(request, "No session years available. Please ask admin to add one.")
    context = {
        "subjects": subjects,
...
def render_staff_leave_form(request):
...
def submit_staff_leave_request(request):
...
def render_staff_feedback_form(request):
    staff_obj = Staffs.objects.get(admin=request.user.id)
...
def submit_staff_feedback(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method.")
...
@csrf_exempt
def fetch_subject_students(request):
...
@csrf_exempt
def persist_attendance_records(request):
...
def render_attendance_update(request):
    subjects = Subjects.objects.filter(staff_id=request.user.id).order_by('subject_name')
    session_years = SessionYearModel.objects.all().order_by('-session_start_year')
    if not subjects.exists():
        messages.warning(request, "No subjects are assigned to your account yet. Please contact admin.")
...
@csrf_exempt
def fetch_attendance_date_options(request):
...
@csrf_exempt
def fetch_attendance_student_rows(request):
    attendance_date = request.POST.get('attendance_date')
...
@csrf_exempt
def persist_attendance_updates(request):
    student_ids = request.POST.get("student_ids")
...
def render_staff_profile(request):
...
def render_result_entry(request):
    subjects = Subjects.objects.filter(staff_id=request.user.id).order_by('subject_name')
    session_years = SessionYearModel.objects.all().order_by('-session_start_year')
    if not subjects.exists():
        messages.warning(request, "No subjects are assigned to your account yet. Please contact admin.")
...
def persist_student_result(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method")
